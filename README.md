...
При запуске `shortener` в stdout выводятся версия, дата и хеш коммита сборки. В `cmd/shortener/main.go` переменные `buildVersion`, `buildDate` и `buildCommit` по умолчанию равны `N/A`; при сборке их можно переопределить флагами линкера `-X`.

Пример сборки:

```
go build -ldflags "-X main.buildVersion=1.0.0 -X main.buildDate=2025-06-03 -X main.buildCommit=abc1234" -o shortener ./cmd/shortener
```
